...
#   
  g <- ametricno2 %>%
    filter(cruise == cruse) %>%
    ggplot(aes(x=mean_range, y=log(NASC), group = cruise)) +
    geom_point(alpha=0.4)+
    scale_x_continuous(limits = c(20, 100))+
    scale_y_continuous(limits = c(1, 12))+
    xlab("90 percentile range") + 
    ylab("log(NASC)")+ 
...
#   
  g <- ametricno2 %>%
    filter(cruise == cruse) %>%
    ggplot(aes(x=mean_dev, y=log(NASC), group = cruise)) +
    geom_point(alpha=0.4)+
...
#   
  g <- ametricno281012 %>%
    ...
    scale_x_reverse()+
    xlab("Mean Absolute Deviation") + 
    ylab("Mean 90th percentile range")+ 
    ggtitle(paste0("Cruise", " ", cruse, " ", "-", " ", seas, " ", yr))
  # print(g)
#   ggsave(png, g)
}
...
ametricno281012.200 = ametricno2[-I,]
...
#   
  g <- ametricno281012.200 %>%
    filter(cruise == cruse) %>%
    ggplot(aes(x=mean_maxsv, y=log(NASC), group = cruise)) +
    geom_point(alpha=0.4)+
    scale_x_continuous(limits = c(4, 64))+
    scale_y_continuous(limits = c(0, 150))+
    scale_x_reverse()+
