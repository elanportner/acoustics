...
##### It looks like there is a decrease in the propotion of signal in the upper 50 and an increase between 50 and 100m. To explore "trend" further, data were extrated from upper 100m in 10m bins.
...
#### Baja = Red, Central = Orange, Northern = light blue, Sonora = Dark Blue
```{r plot_depth_dist_by_region_boxplot, echo = FALSE, fig.width=14, fig.height=20}
acousticdata10 = subset(acousticdata10,Sv_mean > -900)

depth_plot <- function(lyr){ # cntry <- "Canada"
...
#   
  g <- acousticdata10 %>%
    filter(Layer == lyr) %>%
    ggplot(aes(cpoly, Sv_mean, fill = region)) + 
    ...
    ggtitle(paste0("Proportion of NASC in Layer"," ", lyr))+ theme(legend.position="none")
